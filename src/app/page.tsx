...
export default function InsightFlowPage() {
  ...
  const handleFileChange = (file: { name: string; type: string; dataUri: string } | null) => {
    setCurrentFile(file);
    setAnswerData(null); // Clear previous answer when file changes
    if (!file) {
  ...
  const handleQuerySubmit = async () => {
    ...
    try {
      if (currentFile) {
        const result = await analyzeUploadedContent({
          fileDataUri: currentFile.dataUri,
          question: question,
        });
        setAnswerData(result);
      } else {
        const result = await answerWithWebSearch({ question: question });
        setAnswerData(result);
      }
    } catch (error) {
      ...
      setAnswerData({ answer: `Sorry, I encountered an error: ${errorMessage}`, sources: [] });
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <main className="flex-grow container mx-auto p-4 sm:p-6 lg:p-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            <AnswerDisplayCard answerData={answerData} isLoading={isLoading} />
