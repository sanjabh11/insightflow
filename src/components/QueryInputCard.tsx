      ...
      <CardContent className="px-6 pb-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-2">
            <Button type="submit" disabled={isLoading || !question.trim() || isListening} className="flex-grow bg-blue-600 hover:bg-blue-700 text-white text-base font-semibold py-3 rounded-xl shadow-md">
              {isLoading ? (
                <>
            ...
            {speechSupported && (
              <Button type="button" variant="outline" size="icon" onClick={toggleListening} disabled={isLoading} aria-label={isListening ? "Stop listening" : "Start listening"} className="bg-white/80 hover:bg-blue-50 border-0 shadow-none">
                {isListening ? <MicOff className="h-5 w-5 text-destructive" /> : <Mic className="h-5 w-5 text-blue-500" />}
              </Button>
            )}
          </div>
        </form>
