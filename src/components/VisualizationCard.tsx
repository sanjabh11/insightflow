...
function parseCSV(csvString: string): ParsedCsvRow[] {
  ...
  const dataRows = lines.slice(1).map(line => {
    ...
    headers.forEach((header, i) => {
      ...
      row[header] = (value === "" || value === null || value === undefined) ? null : (!isNaN(Number(value)) ? Number(value) : value);
    });
    return row;
  });
  return dataRows;
  ...
  if (validValues.length > 0 && numbers.length / validValues.length > 0.7) { // Heuristic: if 70% are numbers, treat as numerical
    ...
    if (numbers.length > 0) {
      ...
      if (lowerHalf.length > 0) {
        ...
        q1 = lowerHalf.length % 2 !== 0 ? lowerHalf[q1Mid] : (lowerHalf[q1Mid - 1] + lowerHalf[q1Mid]) / 2;
      }
       if (upperHalf.length > 0) {
        const q3Mid = Math.floor(upperHalf.length / 2);
        q3 = upperHalf.length % 2 !== 0 ? upperHalf[q3Mid] : (upperHalf[q3Mid - 1] + upperHalf[q3Mid]) / 2;
      }
    }

    const min = numbers[0] ?? 0;
    const max = numbers[numbers.length - 1] ?? 0;
    const variance = numbers.length > 1 ? numbers.reduce((acc, n) => acc + Math.pow(n - mean, 2), 0) / (numbers.length - 1) : 0;
    const stdDev = Math.sqrt(variance);

    // Histogram data
    const numBins = Math.min(10, Math.max(3, Math.floor(Math.sqrt(numbers.length))));
    ...
    return {
      name: header, type: 'numerical', missing, total: values.length,
    ...
    return {
      ...
      distributionData: sortedCounts.slice(0,10).map(d => ({name: d.value, value: d.count}))
    };
  }
}

function calculateCorrelationMatrix(data: ParsedCsvRow[], numericalColumns: string[]): Record<string, Record<string, number | string>> {
...
const PIE_COLORS = ['#0088FE', '#00C49F', '#FFBB28', '#FF8042', '#8884D8', '#82Ca9D', '#A4De6C', '#D0Ed57', '#FF7F50', '#6495ED'];
...
export function VisualizationCard({ csvDataUri }: VisualizationCardProps) {
  ...
  const [correlationMatrix, setCorrelationMatrix] = useState<Record<string, Record<string, number | string>>>({});
  const [selectedColumnForViz, setSelectedColumnForViz] = useState<string | null>(null);
    ...
    if (csvDataUri) {
      try {
        ...
        if (data.length > 0) {
          ...
          const analyses = currentHeaders.map(header => analyzeColumn(data, header));
          setColumnAnalyses(analyses);
          setSelectedColumnForViz(currentHeaders[0] || null);

          const numericalCols = analyses.filter(a => a.type === 'numerical').map(a => a.name);
          setCorrelationMatrix(calculateCorrelationMatrix(data, numericalCols));

        } else {
          setHeaders([]); setParsedData([]); setColumnAnalyses([]); setCorrelationMatrix({}); setSelectedColumnForViz(null);
        }
      } catch (error) {
  ...
  const numericalHeaders = useMemo(() => headers.filter(h => columnAnalyses.find(ca => ca.name === h && ca.type === 'numerical')), [headers, columnAnalyses]);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Interactive preview and analysis of the uploaded CSV file.</CardDescription>
      </CardHeader>
        ...
        <Accordion type="multiple" defaultValue={['table-preview', 'column-summary']} className="w-full">
                  ...
                  <TableBody>
                    {parsedData.slice(0, 20).map((row, rowIndex) => (
                      <TableRow key={rowIndex}>
                        {headers.map((header) => (
                          <TableCell key={`${rowIndex}-${header}`}>{String(row[header] ?? 'N/A')}</TableCell>
                        ))}
                      </TableRow>
                    ))}
                  </TableBody>
                ...
                <div key={col.name} className="p-3 border rounded-md bg-secondary/30">
                  ...
                  {col.type === 'numerical' && col.numericalStats && (
                    <ul className="text-xs mt-1 space-y-0.5">
                      ...
                      <li>Mean: {col.numericalStats.mean.toFixed(2)}, Median: {col.numericalStats.median.toFixed(2)}</li>
                      <li>Std Dev: {col.numericalStats.stdDev.toFixed(2)}</li>
                       <li>Q1: {col.numericalStats.q1.toFixed(2)}, Q3: {col.numericalStats.q3.toFixed(2)}</li>
                    </ul>
                  )}
                  {col.type === 'categorical' && col.categoricalStats && (
                    <ul className="text-xs mt-1 space-y-0.5">
                      ...
                      {col.categoricalStats.topValues.slice(0,3).map(tv => (
                        <li key={tv.value} className="truncate ml-2">{tv.value}: {tv.count} ({tv.percentage.toFixed(1)}%)</li>
                      ))}
                    </ul>
                  )}
                </div>
          ...
          <AccordionItem value="column-distributions">
            ...
            <AccordionContent className="space-y-4">
              <div className="flex items-center gap-2 mb-2">
                 <Label htmlFor="column-select-dist" className="text-sm">Select Column:</Label>
                 <Select value={selectedColumnForViz || ""} onValueChange={setSelectedColumnForViz}>
              ...
              {selectedColumnAnalysis && selectedColumnAnalysis.distributionData && selectedColumnAnalysis.distributionData.length > 0 && (
                <div>
                  <h4 className="font-semibold text-md mb-2">Distribution of: {selectedColumnAnalysis.name}</h4>
                  <ResponsiveContainer width="100%" height={300}>
                    {selectedColumnAnalysis.type === 'numerical' ? (
                        ...
                        <PieChart>
                            <Pie data={selectedColumnAnalysis.distributionData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} label>
                                {selectedColumnAnalysis.distributionData.map((entry, index) => (
               ...
               {selectedColumnAnalysis && (!selectedColumnAnalysis.distributionData || selectedColumnAnalysis.distributionData.length === 0) && (
                <p className="text-sm text-muted-foreground">No distribution data available or all values are unique/missing for {selectedColumnAnalysis.name}.</p>
               )}
          ...
          {numericalHeaders.length >= 2 && (
            <AccordionItem value="correlation-matrix">
              <AccordionTrigger className="text-lg font-semibold">
                <Percent className="w-5 h-5 mr-2" /> Correlation Matrix (Numerical Columns)
              </AccordionTrigger>
              <AccordionContent>
                <p className="text-xs text-muted-foreground mb-2">Pearson correlation coefficients. Values range from -1 (perfect negative) to 1 (perfect positive). 0 indicates no linear correlation.</p>
                <ScrollArea className="max-h-[400px] w-full border rounded-md">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead className="sticky left-0 bg-card z-10">Column</TableHead>
                        {numericalHeaders.map(h => <TableHead key={h}>{h}</TableHead>)}
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {numericalHeaders.map(rowHeader => (
                        <TableRow key={rowHeader}>
                          <TableHead className="sticky left-0 bg-card z-10">{rowHeader}</TableHead>
                          {numericalHeaders.map(colHeader => {
                            const val = correlationMatrix[rowHeader]?.[colHeader];
                            let bgColor = 'transparent';
                            if (typeof val === 'number') {
                                if (val > 0.7 || val < -0.7) bgColor = 'hsl(var(--primary) / 0.3)';
                                else if (val > 0.4 || val < -0.4) bgColor = 'hsl(var(--primary) / 0.15)';
                            }
                            return (
                                <TableCell key={`${rowHeader}-${colHeader}`} style={{backgroundColor: bgColor}}>
